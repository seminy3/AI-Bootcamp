...
## 3. 로컬 모델 활용하기 (교재 원본 — 참고용)

> ⚠️ **이 파트는 참고용이다.** 이 저장소의 기본 실행 경로는 위 1·2번 API 방식이며, 아래 로컬 방식은 교재 원본을 옮겨 둔 것으로 별도 환경이 필요하다.

인터넷 연결이나 외부 API 의존도를 낮추고 로컬 환경에서 직접 모델을 구동하여 음성을 처리하는 방식이다.

> **실행 환경 주의**: 이 파트는 `torch`·`transformers`·`pyannote.audio 4.x`·`ffmpeg`가 필요하며, `pyannote.audio 4.x`는 **Python 3.10+** 를 요구한다. 교재 환경(Windows, Python 3.12)에서 검증된 코드이며, **이 저장소의 기본 venv(Python 3.9)에서는 위 패키지가 없어 그대로는 실행되지 않는다.** 로컬 파트를 돌리려면 Python 3.10+ 환경에 해당 패키지를 별도 설치해야 한다.
...
>
> **검증 상태**: 1·2번 API 방식(`whisper_api`, `stt_with_speakers`)은 `openai`·`requests`·`pandas`만 있으면 되고 OpenRouter 키 하나로 실제 동작을 확인했다. 3번 로컬 방식은 위 "실행 환경 주의"대로 교재 환경 기준이며 이 저장소 venv에서는 재현되지 않는다.
