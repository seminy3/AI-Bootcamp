...
> **실행 환경 주의**: 이 파트는 `torch`·`transformers`·`pyannote.audio 4.x`·`ffmpeg`가 필요하며, `pyannote.audio 4.x`는 **Python 3.10+** 를 요구한다. 교재 환경(Windows, Python 3.12)에서 검증된 코드이며, **이 저장소의 기본 venv(Python 3.9)에서는 위 패키지가 없어 실행되지 않는다.** 로컬 파트를 돌리려면 Python 3.10+ 환경에 해당 패키지를 별도 설치해야 한다.
